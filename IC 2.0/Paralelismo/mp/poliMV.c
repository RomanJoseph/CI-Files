...
//    #pragma omp parallel for schedule(guided) num_threads(20)
    for (int l = 2; l <= lmax; l++)
    {
      
      sprintf(filename,"./MVs/mv_commander2015_lmax1500_(l=%d).dat", l);

      FILE *MVs = fopen (filename, "a");
...
//  #pragma omp parallel for schedule(guided)
  ...
  {
    sprintf(filename,"alms_commander2015_lmax1500.dat", i);
